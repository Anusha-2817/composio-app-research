...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-bottom:16px">
  <div class="card" style="padding:14px">
    ...
    <div style="font-size:20px;font-weight:700">63 <span class="delta-neg">&rarr; 51</span></div>
  </div>
  <div class="card" style="padding:14px">
...
<p style="font-size:13px;color:var(--fg2);margin-bottom:16px">A deterministic post-processing rule: if <code style="font-family:var(--mono);font-size:12px;background:var(--bg3);padding:1px 5px;border-radius:3px">auth_methods</code> or <code style="font-family:var(--mono);font-size:12px;background:var(--bg3);padding:1px 5px;border-radius:3px">access_tier</code> is not_found, buildability cannot be asserted. This moved 36 apps to insufficient_evidence &mdash; 10 of them from easy_win, the other 26 from buildable_with_caveats and needs_outreach &mdash; taking easy_win from 63 to 53. A second rule, added later: a contact_sales or partner_gated access tier cannot be easy_win with no blocker. That moved Shopify and Waterfall.io to buildable_with_caveats, taking easy_win to 51. Both rules are independent of the citation repair loop &mdash; that one fixed fabricated evidence, these fix unsupported inference. The LLM was asserting easy_win for apps where it had no evidence for the fields that determine buildability.</p>
...
<p><strong>Pattern-based MCP detection systematically misses vendors hosting at separate hostnames.</strong> GitHub, Atlassian, Vercel and Netlify all ship official MCP servers that this pipeline did not find. Three of them publish nothing usable at any probed &lt;domain&gt;/mcp path: github.com/mcp returns a third party&rsquo;s database server, atlassian.com/mcp 404s, and netlify.com/docs/mcp is an 88-character documentation landing page. Vercel failed differently &mdash; it does publish at vercel.com/docs/mcp, the pipeline fetched 1,646 characters from it, and the extractor still returned not_found. Three misses are a retrieval problem; the fourth is an extraction problem. 27 is a floor, not a count.</p>

<p style="margin-top:12px">The deeper problem is that a real quote from a real page can still be about the wrong subject. Ten apps cited a documentation-search server as evidence of a product API server. String matching verifies that a citation exists; it cannot verify that the citation is about the thing being claimed. That gap is structural, and it is the main reason scope is reported as unverified for 16 of the 27.</p>

<p style="margin-top:12px"><strong>Fetcher blind spots.</strong> The link-discovery stage reaches API reference docs reliably but misses pricing pages, MCP documentation, and partner/integration pages. The agent fetched only 4 of 15 pages a human visited for the same 7 apps. GitHub is the clearest case: all 7 pages fetched for it sit under docs.github.com/rest/, while its MCP server documentation &mdash; on the same host, at docs.github.com/en/copilot/ &mdash; was never reached.</p>
